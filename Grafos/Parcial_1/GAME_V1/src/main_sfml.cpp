...
std::string makeInfo(const ggv2::GraphGameV2& game, const GameUiState& ui)
{
    ...
    out << "Modo: " << (ui.mode.empty() ? "-" : ui.mode) << "\n";
    out << "Nodos: " << game.nodes().size() << "\n";
    out << "Componentes: " << game.connectedComponents(true) << "\n\n";

    out << "Categorias\n";
    out << "H hospital | C casa | R refugio\n";
    out << "S super | F farmacia | P policia\n";
    out << "B barricada | X zona roja\n\n";

    if (!ui.mode.empty())
    {
        out << (ui.result.found ? "Ruta activa\n" : "Sin ruta\n");
        if (ui.result.found)
        {
            out << "Costo: " << ui.result.cost << "\n";
...
int main()
{
    const std::string mapFile = resolveExistingPath({
        "GAME_V1/data/progra_villa_v2.txt",
    ...
    while (window.isOpen())
    {
        ...
        while (const std::optional event = window.pollEvent())
        {
                    ...
                    event->getIf<sf::Event::MouseButtonPressed>())
            {
                ...
                if (picked >= 0)
                {
                    ...
                    {
                        const std::string nodeId = game.nodes()[picked].id;
                        if (game.nodes()[picked].redZone)
                        {
        ...
        sf::Text footer = makeText(font,
                                   "Mouse: izq origen | der destino | medio bloquear. Teclado: B DFS? no, B BFS | D DFS | J Dijkstra | Enter ejecuta consola.",
                                   15,
